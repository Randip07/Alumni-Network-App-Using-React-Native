...
export const createEventNotification = async (notification) => {
   try {
      const { data, error } = await supabase.from("eventNotifications").insert(notification).select().single();
...
export const fetchEventNotificationDetails = async (receiverId) => {
   try {
      const { data, error } = await supabase
